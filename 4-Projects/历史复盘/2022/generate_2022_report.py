...
# 读取数据
with open('stats_2022.json', 'r', encoding='utf-8') as f:
    stats = json.load(f)

df_daily = pd.read_csv('sh_index_2022.csv')
df_daily.columns = ['date', 'code', 'open', 'high', 'low', 'close', 'volume', 'amount', 'turnover', 'change_pct']
df_daily['date'] = pd.to_datetime(df_daily['date'])

df_monthly = pd.read_csv('monthly_2022.csv')
...
ax1.set_title('2022年上证指数全年走势：俄乌冲突与美联储加息的至暗时刻', fontsize=16, fontweight='bold', pad=20)
...
plt.tight_layout()
plt.savefig('chart_2022_index.png', dpi=150, bbox_inches='tight')
plt.close()
print("[OK] 生成全年走势图: chart_2022_index.png")
...
ax.set_title('2022年上证指数月度涨跌幅', fontsize=14, fontweight='bold')
...
plt.tight_layout()
plt.savefig('chart_2022_monthly.png', dpi=150, bbox_inches='tight')
plt.close()
print("[OK] 生成月度涨跌幅图: chart_2022_monthly.png")
...
q_returns = [-10.65, 4.50, -11.01, 2.14]
...
bins = [2800, 3000, 3200, 3400, 3600, 3800]
...
plt.suptitle('2022年市场情绪温度计：至暗时刻的坚守', fontsize=16, fontweight='bold', y=0.98)
plt.savefig('chart_2022_sentiment.png', dpi=150, bbox_inches='tight')
plt.close()
print("[OK] 生成情绪温度计: chart_2022_sentiment.png")
