...
# 读取数据
with open('stats_2019.json', 'r', encoding='utf-8') as f:
    stats = json.load(f)

df_daily = pd.read_csv('sh_index_2019.csv')
df_daily.columns = ['date', 'code', 'open', 'high', 'low', 'close', 'volume', 'amount', 'turnover', 'change_pct']
df_daily['date'] = pd.to_datetime(df_daily['date'])

df_monthly = pd.read_csv('monthly_2019.csv')
...
events = [
    ('2019-01-04', '2440\n市场底', 'red'),
    ('2019-02-25', '暴涨5.6%', 'green'),
    ('2019-04-08', '3288\n阶段顶', 'orange'),
    ('2019-05-06', '千股跌停', 'red'),
    ('2019-06-13', '科创板\n开板', 'blue'),
    ('2019-08-06', '2733\n黄金坑', 'purple'),
    ('2019-12-13', '中美协议', 'green'),
]
...
ax1.set_title('2019年上证指数全年走势：从2440点到3050点的科技牛起点', fontsize=16, fontweight='bold', pad=20)
...
plt.tight_layout()
plt.savefig('chart_2019_index.png', dpi=150, bbox_inches='tight')
plt.close()
print("[OK] 生成全年走势图: chart_2019_index.png")
...
ax.set_title('2019年上证指数月度涨跌幅', fontsize=14, fontweight='bold')
...
plt.tight_layout()
plt.savefig('chart_2019_monthly.png', dpi=150, bbox_inches='tight')
plt.close()
print("[OK] 生成月度涨跌幅图: chart_2019_monthly.png")
...
q_returns = [23.93, -1.31, -3.64, 4.99]
...
for i, v in enumerate(q_returns):
    ax4.text(i, v + (0.5 if v > 0 else -0.8), f'{v:.2f}%', ha='center', 
             va='bottom' if v > 0 else 'top', fontsize=11, fontweight='bold')

ax5 = fig.add_subplot(gs[1, 2])
bins = [2400, 2600, 2800, 3000, 3200, 3400]
...
plt.suptitle('2019年市场情绪温度计：科技牛启动', fontsize=16, fontweight='bold', y=0.98)
plt.savefig('chart_2019_sentiment.png', dpi=150, bbox_inches='tight')
plt.close()
print("[OK] 生成情绪温度计: chart_2019_sentiment.png")
