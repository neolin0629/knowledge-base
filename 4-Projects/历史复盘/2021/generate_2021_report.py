...
# 读取数据
with open('stats_2021.json', 'r', encoding='utf-8') as f:
    stats = json.load(f)

df_daily = pd.read_csv('sh_index_2021.csv')
df_daily.columns = ['date', 'code', 'open', 'high', 'low', 'close', 'volume', 'amount', 'turnover', 'change_pct']
df_daily['date'] = pd.to_datetime(df_daily['date'])

df_monthly = pd.read_csv('monthly_2021.csv')
...
ax1.set_title('2021年上证指数全年走势：结构性分化极致之年', fontsize=16, fontweight='bold', pad=20)
...
plt.tight_layout()
plt.savefig('chart_2021_index.png', dpi=150, bbox_inches='tight')
plt.close()
print("[OK] 生成全年走势图: chart_2021_index.png")
...
ax.set_title('2021年上证指数月度涨跌幅', fontsize=14, fontweight='bold')
...
plt.tight_layout()
plt.savefig('chart_2021_monthly.png', dpi=150, bbox_inches='tight')
plt.close()
print("[OK] 生成月度涨跌幅图: chart_2021_monthly.png")
...
for i, v in enumerate(q_returns):
    ax4.text(i, v + (0.2 if v > 0 else -0.3), f'{v:.2f}%', ha='center', 
             va='bottom' if v > 0 else 'top', fontsize=11, fontweight='bold')

ax5 = fig.add_subplot(gs[1, 2])
bins = [3300, 3400, 3500, 3600, 3700, 3800]
...
plt.suptitle('2021年市场情绪温度计：结构性分化极致', fontsize=16, fontweight='bold', y=0.98)
plt.savefig('chart_2021_sentiment.png', dpi=150, bbox_inches='tight')
plt.close()
print("[OK] 生成情绪温度计: chart_2021_sentiment.png")
